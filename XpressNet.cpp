...
*/
//=========================================== XPressNet V.4  Half-Duplex ou Full-Duplex =====================================================
//                             //------ Définisser un mode de transmission dans XpressNet.h --------

// Dernière modif: 01-09-26
//
#include <Arduino.h>
...
bool Debug = false;          // Pas de log. de debugage
//bool Debug = true;             // Log. de debugage
...
#endif //...................................
  if (Bi < 3) return;                                     // Trame trops courte
  if (Debug)  AfficheRx();
  ...
  switch (BufXpress[0]) {
    case 0xE6:  {                                        //   POM CV write MultiMaus
        switch (BufXpress[1]) {
          case 0x30:
            ...
            if (Debug) Serial.println("        POM CV multiMAUS");
            break;
        }
      } break;
    case 0xE4:
      ...
      switch (BufXpress[1]) {
        ...
        case 0x13:                                         // Contrôle vitesse loco. 128 crans
          if (Debug) {
            Serial.print("  Loco. n°: "); Serial.print(xpAdr); Serial.print(" Vit: "); Serial.print(xpSpeed); Serial.print("   ");
          }
      ...
      break;
    case 0xE3:
      xpAdr = word(BufXpress[2] & 0x3F, BufXpress[3]);                                      // 14 bits
      if (Debug) {
        Serial.print("        Demande info. loco. n°: "); Serial.println(xpAdr);
      }
      XPressNet::getfonction(xpAdr);
      switch (BufXpress[1]) {
        case 0x00: {
            xpSpeed = DCC::getThrottleSpeed(xpAdr);
            uint8_t LocoInfo[] = {DirectedOps[Ti], 0xE4, 0x04, xpSpeed, Gr1, Gr2, 0x00 };   // 0xE4  0x04  (F0 à F8)
        ...
        case 0x07: {
            ...
            uint8_t LocoFkt[] = { DirectedOps[Ti], 0xE3, 0x50, 0x00, 0x00, 0x00 };
            XPressNet::getXOR(LocoFkt, 6);
            XNetsend(LocoFkt, 6);
          } break;
        case 0x08: {                  // 3.6  Requête d'état de fonction F13-F28
            ...
            uint8_t LocoFkt[] = { DirectedOps[Ti], 0xE4, 0x51, Gr4, Gr5, 0x0F, 0x00 };          // F13 à F28
            XPressNet::getXOR(LocoFkt, 7);
            XNetsend(LocoFkt, 7);
          } break;
        case 0x09: {
            uint8_t LocoFkt[] = {DirectedOps[Ti], 0xE3, 0x52, Gr4, Gr5, 0x00 };        // F13 à F28
            XPressNet::getXOR(LocoFkt, 6);
            XNetsend(LocoFkt, 6);
          } break;
        case 0xF0:                   // Multimaus
    ...
    case 0x52: {                                         // Requête de commande à un décodeur d’accessoire.
        ...
        if (bitRead (data, 2))
          bitSet(TrntInfo[3], 4);
        memoTrnt = TrntInfo[3];
        if (Debug) Serial.println("        Commande accessoire.");
        getXOR(TrntInfo, 5);
        XNetsend(TrntInfo, 5);
      } break;
    case 0x42: {                                          // 0x42 Demande informations sur le décodeur d'accessoires
        if (Debug) Serial.println("        Demande info. accessoire.");
        int Address = BufXpress[1];   // Modif
        int data = BufXpress[2];
        uint8_t TrntInfo[] = {DirectedOps[Ti], 0x42, 0x00, 0x00, 0x00 }; // Feedback (0x42,MOD,DATA,...,XOR) Accessory decoder information response
        TrntInfo[2] = Address;
    ...
    case 0x23:                                           // Requête d'écriture CV en Mode Direct (CV mode (CV=1..256))
      switch (BufXpress[1]) {
        case 0x12:                                       // Register Mode write request (Register Mode)
        case 0x17:                                       // Paged Mode write request(Paged mode)
          Serial.println("          Non supporté");
          break;
        case 0x16:                                       // Direct Mode CV write request (CV mode)
    ...
    case 0x21:
      switch (BufXpress[1]) {
        case 0x80:                                                // Requête de commande d’arrêt d’urgence
          if (Debug) Serial.println("      Arrêt centrale");
        ...
        case 0x81:                                                // Requête de commande de reprise des opérations
          if (Debug) Serial.println("      Demarrage centrale");
          Railpower = csNormal;
        ...
        case 0x21: {                                              // Demande Version Centrale XpressNet    (0x21 0x21 0x00)
            ...
            //  Ti--;
            if (Debug) {
              Serial.print("        V."); Serial.print((XNetVersion >> 4) & 0x0F); Serial.print("."); Serial.print((XNetVersion & 0x0F) % 10);
              Serial.print(".  Centrale: "); Serial.print(XNetID < 10 ? "LENZ" : "ROCO");
            }
          } break;
      ...
      break;
    default:
      if (Debug)  Serial.println("   Commande non supportée.    ");
...
//--------------------------------------------------------
void XPressNet::AfficheRx() {
  for (int n = 0; n < Bi; n++) {
    if (BufXpress[n] < 10) Serial.print("0");
    Serial.print(BufXpress[n], HEX);                   // Affiche datas Rx
    Serial.print(" ");
  }
}
//--------------------------------------------------------
void XPressNet::AfficheTx() {
  for (int n = 1; n < Bii; n++) {
    if (dataTx[n] < 10) Serial.print("0");
    Serial.print(dataTx[n], HEX);                      // Affiche datas Tx
    Serial.print(" ");
  }
}
//------------------------------------------------------
...
void  XPressNet::Fonction(int a, int b) {                  // 28 fonctions
  if (Debug) {
    Serial.print("  Loco. n°: "); Serial.print(xpAdr); Serial.print(" Fonct: "); Serial.println(fonction);
  }
  for (int i = 0; i < a; i++) {
...
#endif
  pinMode(txPin, OUTPUT);                                // Pin Tx en sortie
...
void XPressNet::getfonction(uint8_t xpAdr) {
  ...
  if (Debug) {
    Serial.print("BitMap Fonctions: ");
    Serial.println(FonctionMap, BIN);
  }
  ...
  //Gr1 = (FonctionMap & 0x1F) ;                 // Groupe 1: 00  00  00  0F  F4 0F3  F2  F1    bit0 à bit4
  Gr2 = (FonctionMap & 0xF00) >> 8;              // Groupe 2: 00  00  00  00  F8  F7  F6  F5    bit8 à bit11
  Gr3 = (FonctionMap & 0xF000) >> 12;            // Groupe 3: F12 F11 F10 F9  00  00  00  00    bit12 à bit15
