...
#define Pin_Rx 12             // Pin 12 In Rx
#define Pin_Tx 13             // Pin 13 Out Tx
#define Pin_Dir 15            // Pin du Max485 (RTS)

// Adresses d'appel périfériques:                p + 0x40 + 000x xxxx   (8 bits)
...
// Adresses de réponses aux périfériques:    1 + p + 0x60 + 000x xxxx   (9 bits)
...
void  XpressNetBegin() {
  ...
  uart_set_pin(UART_NUM_2, Pin_Tx, Pin_Rx, Pin_Dir, UART_PIN_NO_CHANGE);
  uart_driver_install(UART_NUM_2, 127 * 2, 0, 0, NULL, 0);
  uart_set_mode(UART_NUM_2, UART_MODE_RS485_HALF_DUPLEX);

  minAdress = 0;                                //  Plage d'adressage des périfériques,
...
void XpressNet() {
  ...
  if ((currentTime - previousTime) > 10) {       // 5 ms minimum
    ...
    Serial2.write(Adress40[Ti]);                // Envoi adresse aux périfériques (A = adresse, P = Parité.   P10A AAAA
  }
}
  ...
  */
  switch (BufXpress[0]) {
    ...
    case 0x23:                                           // Requête d'écriture CV en Mode Direct (CV mode (CV=1..256))
      if (BufXpress[1] == 0x16) {
        // Serial.println("Ecriture CV.");               // DEBUG
      ...
      switch (BufXpress[1] ) {
        ...
        case 0x10:                                         // Lecture du CV Service mode
          if (stateCV == StateCV::Reading_OK) {
            ...
            //Serial.println("Réponse Cv");                 // DEBUG
            DeconnectePin();
            Tx9(Adress60[Ti]);
            Tx9(0x63);
            Tx9(0x15);
...
void DeconnectePin() {
  cli();
  gpio_matrix_out(Pin_Dir, 0x100, false, false);          // Déconnecte la pin Dir (RTS) du module UART2
  gpio_matrix_out(Pin_Tx, 0x100, false, false);           // Déconnecte la pin Tx du module UART2
  digitalWrite(Pin_Dir, HIGH);                            // Dir Haut
  delayMicroseconds(8);                                   // 1/2 bit
}
void ConnectePin() {
  digitalWrite(Pin_Dir, LOW);                             // Dir Bas
  gpio_matrix_out(Pin_Tx, U2TXD_OUT_IDX, false, false);   // Reconnecte la pin Tx au module UART2
  gpio_matrix_out(Pin_Dir, U2RTS_OUT_IDX, false, false);  // Reconnecte la pin Dir (RTS) au module UART2 (RTS)
  sei();
}

void Tx9(int dTx) {                // Envoi d'un octet 62500 Bauds  9bits  1stop
  digitalWrite(Pin_Tx, LOW);       // Bit Start
  delayMicroseconds(15);           // Largeur 16µs
  for (n = 0; n < 9; n++) {        // 9 bits
    if (bitRead(dTx, n))           // Test état du bit
      digitalWrite(Pin_Tx, HIGH);  // Etat haut
    else                           // Si non
      digitalWrite(Pin_Tx, LOW);   // Etat bas
    delayMicroseconds(15);         // Largeur 16µs
  }                                // Boucle
  digitalWrite(Pin_Tx, HIGH);      // 1 Bit Stop
